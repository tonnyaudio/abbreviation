...
class RankerDataset(Dataset):
    def __init__(self, references, contexts, labels, config):
        ...
        self.labels = labels
        # self.ref_encoded = config.tokenizer(references, padding=False, truncation=True)
        # self.context_encoded = config.tokenizer(contexts, padding=False, truncation=True)
...
class RankerOrderDataset(Dataset):
    def __init__(self, references, contexts, labels,orders, config):
        super(RankerOrderDataset, self).__init__()
        self.config = config
        self.references = references
        self.contexts = contexts
        self.labels = labels
    ...
    def __getitem__(self, index):
        ref_encoded = self.config.tokenizer(self.references[index], padding=False, truncation=True, max_length=self.config.max_len - 2)
        context_encoded = self.config.tokenizer(self.contexts[index], padding=False, truncation=True, max_length=self.config.max_len - 2)
        return {'reference': ref_encoded, 'context': context_encoded, 'label': self.labels[index], 'order': self.orders[index]}
...
#         keys = ['input_ids', 'token_type_ids', 'attention_mask',]
        ...
        orders = torch.tensor([item['order'] for item in data], dtype=torch.float64)
        # orders=torch.tensor([item['order'] for item in data], dtype=torch.long)
        return {
            'reference': ref_encoded,
            'context': context_encoded,
            'labels': labels,
...
if __name__ == '__main__':
    config = RankerConfig()
    references, contexts, labels = read_ranker_data('data/ranker_train.txt')
